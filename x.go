...
func xloadcols() {
	...
	dc.col = make([]Color, mathutil.Max(len(colorname), 256))
...
func xloadfont(f *Font, pattern *fc.Pattern) bool {
	...
	f.width = mathutil.Divceil(extents.XOff(), len(ascii_printable))
...
func main() {
	...
	cols = mathutil.Max(cols, 1)
	rows = mathutil.Max(rows, 1)
	tnew(cols, rows)
	xinit(cols, rows)
	xsetenv()
	selinit()
