...
func selextend(col, row, typ int, done bool) {
	...
	if oldey != sel.oe.y || oldex != sel.oe.x || oldtype != sel.typ || sel.mode == SEL_EMPTY {
		tsetdirt(mathutil.Min(sel.nb.y, oldsby), mathutil.Max(sel.ne.y, oldsey))
	}
...
func selnormalize() {
	if sel.typ == SEL_REGULAR && sel.ob.y != sel.oe.y {
		...
		sel.nb.x = mathutil.Min(sel.ob.x, sel.oe.x)
		sel.ne.x = mathutil.Max(sel.ob.x, sel.oe.x)
	}
	sel.nb.y = mathutil.Min(sel.ob.y, sel.oe.y)
	sel.ne.y = mathutil.Max(sel.ob.y, sel.oe.y)
...
func tsetdirt(top, bot int) {
	top = mathutil.Clamp(top, 0, term.row-1)
	bot = mathutil.Clamp(bot, 0, term.row-1)
...
func tscrolldown(orig, n int) {
	n = mathutil.Clamp(n, 0, term.bot-orig+1)
...
func tmoveto(x, y int) {
	...
	term.c.x = mathutil.Clamp(x, 0, term.col-1)
	term.c.y = mathutil.Clamp(y, miny, maxy)
}
...
func tclearregion(x1, y1, x2, y2 int) {
	...
	x1 = mathutil.Clamp(x1, 0, term.col-1)
	x2 = mathutil.Clamp(x2, 0, term.col-1)
	y1 = mathutil.Clamp(y1, 0, term.row-1)
	y2 = mathutil.Clamp(y2, 0, term.row-1)
...
func tdeletechar(n int) {
	n = mathutil.Clamp(n, 0, term.col-term.c.x)
...
func tresize(col, row int) {
	minrow := mathutil.Min(row, term.row)
	mincol := mathutil.Min(col, term.col)
...
func tsetscroll(t, b int) {
	t = mathutil.Clamp(t, 0, term.row-1)
	b = mathutil.Clamp(b, 0, term.row-1)
	if t > b {
		t, b = b, t
	}
	term.top = t
...
func getsel() []byte {
	...
	for y := sel.nb.y; y <= sel.ne.y; y++ {
		...
		lasti := mathutil.Min(lastx, linelen-1)
...
func tdumpline(n int) {
	...
	bp := term.line[n]
	end := mathutil.Min(tlinelen(n), term.col) - 1
	if end > 0 || bp[0].u != ' ' {
...
func draw() {
	...
	cx := term.c.x
	term.ocx = mathutil.Clamp(term.ocx, 0, term.col-1)
	term.ocy = mathutil.Clamp(term.ocy, 0, term.row-1)
